...
public class SkipManager : MonoBehaviour
{
    ...
    [Header("CPR 튜토리얼용 GameManager 참조")]
    ...
    {
        Debug.Log("[SkipManager] SkipAllTutorials() 실행됨");  // ✅ 이게 콘솔에 뜨는지 확인!
        foreach (var canvas in tutorialCanvases)
        {
            if (canvas != null)
                canvas.SetActive(false);
        }
        Debug.Log("튜토리얼 전체 스킵 완료");
        SkipFlags.cprTutorialSkipped = true; // ✅ 추가
        //  CPR 튜토리얼인 경우 GameManager에 반영
        if (SceneManager.GetActiveScene().name == "4_CPR" )
            Debug.Log("[SkipManager] 현재 씬은 4_CPR");
        if (gameManager != null)
            Debug.Log("[SkipManager] gameManager 연결됨");
        if (gameManager != null && gameManager.tutorial)
        {
            Debug.Log("[SkipManager] tutorial == true → 종료 시도");
            ...
            Debug.Log("[SkipManager] CPR 튜토리얼 강제 종료 및 일반 모드 전환");
        }
    }
}